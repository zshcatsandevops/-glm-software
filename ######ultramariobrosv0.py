...
class Player:
    ...
    def jump(self):
        if not self.is_jumping and not self.sliding_down:
    ...
    def update(self, platforms, enemies):
        ...
        if self.sliding_down:
            self.slide_progress += 0.02
            if self.slide_progress >= 1.0:
                self.sliding_down = False
                self.slide_progress = 0
                return True  # Signal that sliding is complete
            else:
                # Move player down the flagpole
                self.y += 3
                # Move player slightly to the right to simulate sliding
                self.x += 0.5
        return False
...
# Flag class (SMB1-style but in 3D Land style)
class Flag:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        ...
        self.castle_x = x + 100
        self.castle_y = y + 100
        self.castle_width = 80
        self.castle_height = 100
    ...
    def draw(self, screen):
        ...
        if not self.collected:
            # Flag pole top
            pygame.draw.circle(screen, (200, 200, 200), (self.x + self.width//2, self.y), 8)
            
            # Flag
            pygame.draw.polygon(screen, RED, [
                (self.x + self.width, self.y + 20),
                (self.x + self.width + self.flag_width, self.y + 30),
                (self.x + self.width + self.flag_width, self.y + 30 + self.flag_height),
                (self.x + self.width, self.y + 20 + self.flag_height)
            ])
            
            # Flag pattern (3D Land style)
            pygame.draw.circle(screen, WHITE, (self.x + self.width + 20, self.y + 50), 8)
            
        # Draw castle
        # Castle base
        pygame.draw.rect(screen, GRAY, (self.castle_x, self.castle_y + 40, self.castle_width, self.castle_height - 40))
        # Castle towers
        pygame.draw.rect(screen, GRAY, (self.castle_x, self.castle_y, 20, 60))
        pygame.draw.rect(screen, GRAY, (self.castle_x + 30, self.castle_y, 20, 60))
        pygame.draw.rect(screen, GRAY, (self.castle_x + 60, self.castle_y, 20, 60))
        # Castle door
        pygame.draw.rect(screen, BLACK, (self.castle_x + 30, self.castle_y + 60, 20, 40))
        # Castle windows
        pygame.draw.rect(screen, YELLOW, (self.castle_x + 5, self.castle_y + 20, 10, 10))
        pygame.draw.rect(screen, YELLOW, (self.castle_x + 35, self.castle_y + 20, 10, 10))
        pygame.draw.rect(screen, YELLOW, (self.castle_x + 65, self.castle_y + 20, 10, 10))
...
world_levels_completed = [False, False, False, False, False]  # Track if all levels in each world are completed

# Create levels with SMBX-style design
def create_level(world, level):
    global platforms, enemies, coins, powerups, flag, boss, camera_x
    ...
    if level < 4:  # Regular levels
        ...
        # Add flag at the end (SMB1-style but in 3D Land style)
        flag = Flag(level_length - 250, SCREEN_HEIGHT - 250)
...
def create_world_map():
    ...
    for level in range(1, 5):
        ...
        completed = (level < current_level) or (level == current_level and world_levels_completed[current_world-1])
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_SPACE:
                ...
                current_level = 1
                selected_world = 1
                world_levels_completed = [False, False, False, False, False]
                create_level(current_world, current_level)
                create_world_map()
                create_main_overworld()
                game_state = MAIN_OVERWORLD
    ...
    if game_state == INTRO:
        ...
        # Update game objects
        sliding_complete = player.update(platforms, enemies)
        ...
        if flag and not player.flag_reached:
            if (player.x + player.width > flag.x and 
                player.x < flag.x + flag.width and
                player.y + player.height > flag.y):
                player.flag_reached = True
                player.sliding_down = True
                flag.collected = True
                player.score += 1000  # Flag completion bonus
                
        # Handle flag sliding completion
        if sliding_complete:
            level_complete = True
            level_timer = 0
            # Mark current level as completed
            if current_level == 4:  # If it's the last level in the world
                world_levels_completed[current_world-1] = True
        ...
        if level_complete:
            ...
            if level_timer > 120:  # 2 seconds delay
                ...
                if current_level < 4:
                    ...
                    # All levels in world completed, go to boss
                    current_level = 1
                    game_state = BOSS
                create_level(current_world, current_level)
                create_world_map()
        ...
        # Draw flag if in view
        if flag and flag.x > camera_x and flag.x < camera_x + SCREEN_WIDTH:
            flag.draw(screen)
